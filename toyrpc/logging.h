...
inline void SimpleLogger(int level, const char *component, const char *filename,
                         int lineNum, int enabled_level, const char *format,
                         ...) {
...
#define REMMY_LOG(format, ...)                                                 \
  do {                                                                         \
    SimpleLogger(REMMY_LOG_INFO, LOGGING_COMPONENT, __FILE__, __LINE__,        \
                 REMMY_LOG_LEVEL, (format), ##__VA_ARGS__);                    \
  } while (0)
#else
#define REMMY_LOG(...)
#endif
...
#define REMMY_WARN(format, ...)                                                \
  do {                                                                         \
    SimpleLogger(REMMY_LOG_WARNING, LOGGING_COMPONENT, __FILE__, __LINE__,     \
                 REMMY_LOG_LEVEL, (format), ##__VA_ARGS__);                    \
  } while (0)
#else
#define REMMY_WARN(...)
...
#define REMMY_ABORT(format, ...)                                               \
  do {                                                                         \
    SimpleLogger(REMMY_LOG_ERROR, (LOGGING_COMPONENT), (__FILE__), (__LINE__), \
                 REMMY_LOG_LEVEL, (format), ##__VA_ARGS__);                    \
  } while (0)
#else
...
#define REMMY_ASSERT(pred, format, ...)                                        \
  do {                                                                         \
    if (!(pred))                                                               \
      REMMY_ABORT((format), ##__VA_ARGS__);                                    \
  } while (0)
#else
#define REMMY_ASSERT(...)
#endif
...
#define SIMPLE_SUICIDE(format, ...)                                            \
  do {                                                                         \
    SimpleLogger(REMMY_LOG_WARNING, (LOGGING_COMPONENT), (__FILE__),           \
                 (__LINE__), REMMY_LOG_LEVEL, (format), ##__VA_ARGS__);        \
    exit(0);                                                                   \
  } while (0)
